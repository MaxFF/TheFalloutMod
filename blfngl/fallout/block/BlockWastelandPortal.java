...
import blfngl.fallout.world.DimensionWastelandTeleporter;
import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

public class BlockWastelandPortal extends BlockBreakable
{
	public BlockWastelandPortal(int par1)
	{
	...
	public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
	{
		...
		else
		{
			...
			for (l = -1; l <= 2; ++l)
			{
				for (i1 = -1; i1 <= 3; ++i1)
				{
					...
					if (l != -1 && l != 2 || i1 != -1 && i1 != 3)
					{
						...
						else if (j1 != 0 && j1 != Fallout.blockPortalActivator.blockID)
						{
							return false;
						}
					}
			...
			for (l = 0; l < 2; ++l)
			{
				for (i1 = 0; i1 < 3; ++i1)
				{
					par1World.setBlock(par2 + b0 * l, par3 + i1, par4 + b1 * l, Fallout.portalWateland.blockID, 0, 2);
				}
			}
			return true;
		}
	...
	public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity)
	{
		if ((par5Entity.ridingEntity == null) && (par5Entity.riddenByEntity == null) && ((par5Entity instanceof EntityPlayerMP)))
		{
			...
			else if (thePlayer.dimension != Fallout.dimensionWasteland)
			{
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, Fallout.dimensionWasteland, new DimensionWastelandTeleporter(thePlayer.mcServer.worldServerForDimension(Fallout.dimensionWasteland)));
			}
			else {
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, 0, new DimensionWastelandTeleporter(thePlayer.mcServer.worldServerForDimension(0)));
			}
		}
	}
	@SideOnly(Side.CLIENT)
